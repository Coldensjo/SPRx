...
- **Full app / UI change** → build the portable exe and drive the real Windows GUI with `.claude/skills/run-sprx/driver.ps1` (PowerShell + Win32 API — no tmux/xvfb needed, this is a native Windows app, not headless).
...
```sh
pwsh -File .claude/skills/run-sprx/driver.ps1 launch                          # starts sprx-portable.exe, ~3s
pwsh -File .claude/skills/run-sprx/driver.ps1 screenshot out.png              # crops to just the app window
pwsh -File .claude/skills/run-sprx/driver.ps1 openfile 'C:\path\Tibia.dat'    # Ctrl+O, types path, Enter
pwsh -File .claude/skills/run-sprx/driver.ps1 click <x> <y>                   # coords relative to app window
pwsh -File .claude/skills/run-sprx/driver.ps1 keys '{ESC}'                    # raw SendKeys string
pwsh -File .claude/skills/run-sprx/driver.ps1 rect                            # print window screen rect (debugging)
pwsh -File .claude/skills/run-sprx/driver.ps1 close                          # kills sprx-portable.exe + sprx.exe
```
